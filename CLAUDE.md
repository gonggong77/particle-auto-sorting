...
- **Hierarchy Sibling Reorder (수정 모드)**: 같은 부모 자식 사이만, 행 좌측 `≡` 핸들 드래그앤드롭. 같은 transform 의 모든 RendererInfo (PS+Trail) 동반 이동. 메모리 (`HierarchyOrder` mutate) 즉시 반영, 디스크는 [적용] 시 `SetSiblingIndex` + `SavePrefabAsset` 일괄. [재계산] 버튼은 수정 모드 ON 일 때 [미리보기] 라벨로 토글되며 동작은 동일 (`RecomputeAll` — 디스크 무변경). reorder 후 미리보기 미클릭 행에는 "↻ 미리보기 필요" 뱃지.
